...
def _allocate_locked(
    ...
    *,
    ...
    if now >= grant.expires_at_utc:
        # §23.3: an expired approval is re-derived, never executed late. The
        # grant inherits its request's expiry precisely so that an approval
        ...
        raise PromotionError(
            ...
            "requires the action be regenerated and re-evaluated rather than executed "
            "on stale terms"
        )
